# -*- coding: utf-8 -*-
"""
Created on Mon Jan 18 21:00:51 2016
...
class Hubbard_SpinWGenerator:
    ...
    def __init__(self,latticedim,mag,bonds,hopping=[],hubbard=[],terms=[]):
        '''
        Constructor.
        Parameter:
            bonds: list of Bond
        ...
        self.parameters={}
        self.terms={}
        self.Uterms=HuBbardList(*hubbard)
        self.Ubonds=self.Uterms.bond_2nd_order(self.bonds)
        self.Sterms=SuperexchangeList(*terms)
        self.Sbonds=self.Sterms.bondclassify(self.bonds)
        self.set_parameters_and_terms(hopping,hubbard,terms)
        self.cache={}
        self.set_cache()
        self.findCLSE()
        self.linearoperators()

    def set_parameters_and_terms(self,hps,hus,terms):
        self.parameters['const']=OrderedDict()
        self.parameters['alter']=OrderedDict()
        self.terms['U']={}
        self.terms['S']={}
        if len(hps)==0:
            self.parameters['const']['t']='I'
        else:
            self.terms['t']={}
            self.terms['t']['alter']={}
            self.terms['t']['const']=HoPpingList()
            for term in hps:
                if hasattr(term,'modulate'):
                    self.parameters['alter'][term.tag]=term.value
                    self.terms['t']['alter'][term.tag]=HoPpingList()
                    self.terms['t']['alter'][term.tag].append(term)
                    self.terms['t']['const'].append(term)
                else:
        ...
        if len(hus)!=0:
            ...
            for term in hus:
                if hasattr(term,'modulate'):
                    self.parameters['alter'][term.tag]=term.value
                    self.terms['U']['alter'][term.tag]=+term
                else:
                    self.parameters['const'][term.tag]=term.value
                    self.terms['U']['const'][term.tag]=+term
        if len(terms)!=0:
            self.terms['S']['alter']={}
            self.terms['S']['const']={}
            for term in terms:
                if hasattr(term,'modulate'):
                    self.parameters['alter'][term.tag]=term.value
                    self.terms['S']['alter'][term.tag]=+term
                else:
                    self.parameters['const'][term.tag]=term.value
                    self.terms['S']['const'][term.tag]=+term
 
    def per_2nd_order(self):
        self.origin_cache=SpinWmatrixList()
        bonds=self.Ubonds
        if 'const' in self.terms['U']:
            for tag,term in self.terms['U']['const'].iteritems():
                for bond in bonds[tag]:
                    self.origin_cache.append(term[0].spinWmat(bond))
        if 'alter' in self.terms['U']:
            self.U_cache={}
            for tag in self.terms['U']['alter'].iterkeys():
                self.U_cache[tag]=SpinWmatrixList()
                for bond in bonds[tag]:
                    temp=self.terms['U']['alter'][tag][0].spinWmat(bond)
                    self.U_cache[tag].append(temp)
                    
    def transform(self):
        for spinWM in self.origin_cache:
            self.terms['t']['const'].spinW(spinWM)
        for spinWM in self.U_cache.values():
            self.terms['t']['const'].spinW(spinWM)

    def set_cache(self):
        self.per_2nd_order()
        if not hasattr(self.terms,'t'):
            self.transform()
        bonds=self.Sbonds  
        if 'const' in self.terms['S']:
            self.cache['const']=SpinWmatrixList()
            for tag in self.terms['S']['const'].iterkeys():
                for bond in bonds[tag]:
                    self.cache['const'].append(self.terms['S']['const'][tag].spinWmat(bond))
        if 'alter' in self.terms['S']:
            self.cache['alter']={}
            for tag in self.terms['S']['alter'].iterkeys():
                self.cache['alter'][tag]=SpinWmatrixList()
                for bond in bonds[tag]:
                    self.cache['alter'][tag].append(self.terms['S']['alter'][tag].spinWmat(bond))
    ...
    def _classicalE(self,x):
        ...
        if hasattr(self,'U_cache'):
            for cache in self.U_cache.itervalues():
                for SpinWM in cache:
                    energy=energy+SpinWM.testupdate(rotate)         
...
#        return result
    ...
    def update(self,**karg):
        ...
        '''
        if 'alter' in self.terms['t']:
            maskst=False
            for key,term in self.terms['t']['alter'].iteritems():
                ...
                if masks:
                    self.transform()
        if 'alter' in self.terms['U']:
            masksu={key:False for key in self.terms['U']['alter'].iterkeys()}
            for key,term in self.terms['U']['alter'].iteritems():
                nv=term[0].modulate(**karg)
                if nv is not None and norm(array(nv)-array(term[0].value))>RZERO:
                    masku[key]=True
                    term[0].value=nv
                    self.parameters['alter'][key]=nv
            for tag,mask in masksu.iteritems():
                if mask:
                    self.U_cache[tag]=SpinWmatrixList()
                    for bond in bonds[tag]:
                        temp=self.terms['U']['alter'][tag].spinWmat(bond)
                        self.U_cache[tag].append(temp)
        if 'alter' in self.terms['S']:
            masks={key:False for key in self.terms['S']['alter'].iterkeys()}
            for key,term in self.terms['S']['alter'].iteritems():
                nv=term[0].modulate(**karg)
                if nv is not None and norm(array(nv)-array(term[0].value))>RZERO:
                    masks[key]=True
                    term[0].value=nv
            ...
            if maskst&any(masksu.values())&any(masks.values()):
                self.findCLSE()
                self.linearoperators()
