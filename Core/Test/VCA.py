...
def test_vca():
    ...
    a=VCA(
            ...
            nambu=      False,
            weiss=[     Onsite('afm',0.00,indexpackages=sigmaz('sp'),amplitude=lambda bond: 1 if bond.spoint.site in (0,3) else -1,modulate=lambda **karg:karg['afm'])]
            )
    ...
    a.addapps('GFC',GFC(nstep=200,save_data=False,method='user',vtype='RD',error=10**-10,run=ONRGFC))
    ...
    a.addapps('OP',OP(terms=a.weiss+[Onsite('N',1.0)],BZ=square_bz(reciprocals=a.lattice.reciprocals,nk=100),run=VCAOP))
