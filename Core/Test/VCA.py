...
def test_vca():
    ...
    #a.addapps('CP',CP(BZ=square_bz(reciprocals=a.lattice.reciprocals,nk=100),eta=0.01,a=0,b=U,run=VCACP))
    #a.addapps('OP',OP(terms=a.weiss,BZ=square_bz(reciprocals=a.lattice.reciprocals,nk=100),run=VCAOP))
    a.addapps('FF',FF(BZ=square_bz(reciprocals=a.lattice.reciprocals,nk=100),run=VCAFF))
    a.runapps()
